...
def _fit_kmeans_2d(X: np.ndarray, k: int, *, seed: int = 0) -> object:
    ...
    if _SklearnKMeans is not None:
        # Use a conservative n_init for broad sklearn compatibility.
        model = _SklearnKMeans(n_clusters=k, random_state=seed, n_init=10)
        model.fit(X)
        return model

    # Fallback: Lloyd iterations with random init
...
class OracleDifficultyModelKMeans:
    ...
    kmeans: object
    # Ordered by "easiness" score (higher A(1) first) for stable printing / budgets.
    centers_ab: np.ndarray  # (k,2) => [a,b]
    probs: np.ndarray  # (k,)
    # Mapping from raw cluster label -> ordered bucket index
    cluster_to_bucket: np.ndarray  # (k,)
...
def build_oracle_difficulty_model(
    ...
    *,
    ...
    kmeans = _fit_kmeans_2d(X, k_eff, seed=random_seed)
    # sklearn: has labels_ and cluster_centers_; fallback: we compute labels via predict
    ...
    # Stable ordering: sort by A_probit(1) descending (easy -> hard).
    scores = np.asarray([base.A_probit(1, float(a), float(b)) for a, b in centers], dtype=float)
    perm = np.argsort(-scores)  # descending
...
def greedy_budget_allocation_oracle(
    ...
    *,
    ...
    # If already at/over target (possible when min_budget is large), keep the hard constraint.
    # If infeasible (avg < E[min_budget]), report best-effort baseline.
    ...
    # Phase A: gain-driven greedy, but only apply feasible moves (do not break on infeasible).
    ...
    # Phase B: fill-to-tight. Ignore gain and pack remaining slack using the smallest-cost
    # (smallest prob) feasible increments, without ever exceeding average_budget.
